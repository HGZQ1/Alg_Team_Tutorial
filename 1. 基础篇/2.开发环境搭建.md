...
更多命令可以查看[这个](https://harry-hhj.github.io/posts/Linux-Commands/)。 

Ubuntu可能不会自带pip（一种python包管理工具）,需自己安装： 
```bash
sudo apt update
...
<img src="https://sabe.io/tutorials/learn-git/hero.png" alt="ROS2 humble" style="width: 300px; height: 200px;">

### 2.3.1 Git 是什么

Git 是一个**版本控制工具**，用于记录代码和文档的修改历史。它可以帮助我们查看差异、保留可运行版本，并让多人协作时不互相覆盖文件。Git可以这么理解就是它是一个记录员，你每更新一版项目他就会将你完整的项目和更改记录下来放在一个文件夹里，并给你保存管理好，你可以随时向他要项目的每个版本和查看修改，是我们在开发时的时光机和后悔药。

毕竟完成一个算法的优化不是一直一个文件往前走的，我们需要对比往期方案，分析当前和之前的方案；而且，有时候算法或者参数出现问题，通过比对正常运行的前一版也能更快发现问题。
...
`commit` （提交）只保存到本地，`push`（推送） 才会上传到 GitHub；`pull`（拉取） 用于获取队友已经上传的更新。

### 2.3.2 安装与配置 Git

在 Ubuntu 22.04 中打开终端，执行：
...
能看到版本号就说明安装成功。第一次使用 Git 时设置提交信息：
...
`user.name` 和 `user.email` 是提交记录中的作者信息，不是 GitHub 密码。向 GitHub 推送时需要相应的仓库写权限；按团队要求完成 GitHub 登录即可，**不要把密码和令牌写进代码或仓库地址**。

### 2.3.3 第一次获取仓库（clone）

`git clone` 只在第一次获取项目时使用。以 RC2026 仓库为例：

```bash
mkdir -p ~/Projects
cd ~/Projects
git clone https://github.com/HGZQ1/Alg_Team_Tutorial.git 

cd Alg_Team_Tutorial
git remote -v
git status
```

仓库地址以 GitHub 项目页面 **Code** 按钮显示的地址为准。克隆完成后，项目文件就在当前目录中，后续不需要再次 `clone`。

### 2.3.4 拉取、提交与推送

开始工作前，先进入项目目录并拉取最新代码：
...
| 命令 | 作用 |
| --- | --- |
| `git pull` | 获取并整合远程仓库的新提交 |
| `git status` | 查看当前文件状态 |
| `git diff` | 查看尚未提交的具体修改 |
| `git add` | 选择本次要提交的文件 |
| `git commit` | 保存一个本地版本 |
| `git push` | 将本地提交上传到远程仓库 |

提交前建议检查 `git status`，确认没有把编译产物、日志或无关文件一起提交。`git add .` 会暂存当前目录下的全部修改，新生应谨慎使用。

如果出现“没有权限推送”的提示，说明当前 GitHub 账号不是该仓库的协作者；先联系仓库负责人，或在自己的 Fork 中提交。

### 2.3.5 ROS2 项目的 `.gitignore`

ROS2 编译产生的 `build`、`install` 和 `log` 目录可以重新生成，通常不应提交，提交了纯占地方。可以在项目根目录创建 `.gitignore`：
...
`.vscode/` 是否加入忽略列表由团队决定：需要共享调试配置时保留它，仅个人配置时再忽略。源码、`package.xml`、`setup.py`、启动文件和必要的配置文件应当提交。提交前检查是否包含密码、私钥、日志、数据集或不应公开的模型文件。

### 2.3.6 在 VS Code 中使用 Git

VS Code 的 Git 功能依赖系统中已经安装的 Git。
打开仓库根目录：

```bash
cd ~/Projects/Alg_Team_Tutorial
code .
```

在 VS Code 左侧点击**源代码管理**图标，或按 `Ctrl+Shift+G`：

<img src="../picture1/2026-08-20 163945.png" alt="ROS2 humble" style="width: 200px; height: 420px;">

1. 在 **Changes(更改)** 中点击文件，查看修改差异。 
 
   <img src="../picture1/2026-08-20 164308.png" alt="ROS2 humble" style="width: 300px; height: 320px;"> 
   （红色是修改前的，绿色是修改后的） 

3. 在“更改”内点击文件旁的 `+`（暂存更改），将需要提交的文件加入暂存区。
4. 在提交说明框输入本次修改内容（也就是“消息”，但个人不建议在实际开发时像我那样乱写，老老实实说改了哪里），点击 **Commit（提交）**。
5. 提交完成后，点击 **Sync Changes（同步更改）** 同步远程，同步github仓库的最新版本项目（也就是其他同学写的代码也会同步到你的项目中）；也可以在 `...` 菜单中单独执行 **Pull** 或 **Push**。也就是把修改后的版本上传到Github。
6. 修改分支和仓库前，先确认窗口打开的是项目根目录。

VS Code 按钮和终端命令操作的是同一个 Git 仓库，两种方式可以交替使用。提交前仍要检查文件列表，避免误提交生成文件或个人密钥。

### 2.3.7 本节练习

1. 安装并配置 Git。
2. 将当前教程仓库克隆到 `~/Projects`。
3. 尝试了解 VS Code 的源代码管理面板。

（有能力就自己在Github上建个仓库练手，尝试完成拉取，修改，提交，推送）

推荐记住这条最小流程：

```bash
git pull --rebase      # 开工前，减少冲突
# ...改文件...
git status
git add 要提交的文件
git commit -m "清楚描述本次修改(乱写的被我知道后果自负)"
git pull --rebase      # 推之前再拉一次
git push
``` 
## 2.4 SSH配置 
我们做开发完一个版本的上位机系统后就需要部署到真机上面进行实际测试和参数调试，正常人都是在自己电脑开发，然后最终部署到另外一台电脑。此时我们调试的时候，修改参数，修改代码，查看日志什么的就非常不方便了，你也不想追着一台正在动的小车后面的小显示屏看日志吧。SSH就是算法组在进行调车的时候常用的远程方式。 

在上位机上开启 SSH 服务,注意：**开发机和部署机都要安装！！！** 
```bash
# 安装 openssh-server
...
# 查看上位机的 IP 地址
ip addr show        #或者ifconfig,找到最下面那里192.XX.XX.X或10.xx.xxx.xxx的那个就是了,这是内网IP
``` 
<img src="../picture1/e0ea803b782019959861608c6dfa9dff.jpg" alt="ROS2 humble" style="width: 300px; height: 350px;">

从你的电脑连接到上位机
```bash
# 基本格式
ssh username@ip_address #上位机(小电脑)的username和IP，然后回车输入上位机的密码
...
## 配置VScode的SSH 
上面配置好的SSH是远程连接终端，在终端运行一些基本的指令或者查看一些输出还是非常方便的；但是你说如果要修改参数修改代码就非常麻烦了。所以我们直接在VScode里面配置SSH，直接远程连接上位机的VScode!!! 

打开你电脑的VScode，在扩展里面输入“SSH”，找到Remote-SSH安装 

 <img src="../picture1\屏幕截图 2026-08-30 215759.png" alt="ROS2 humble" style="width: 300px; height: 200px;"> 

安装完成后在最左侧找到一个像显示屏的标志(远程资源管理器)，点进去找到“+”添加远程，然后在最上面的输入框输入`ssh username@ip_address`和密码即可远程连接到开发机。【但是不建议用这种方式直接在真机进行开发】 

 <img src="../picture1\屏幕截图 2026-08-30 215842.png" alt="ROS2 humble" style="width: 650px; height: 250px;"> 

但是至于openCV可视化，rviz,rqt,这些如果在调车过程中需要看着图像，那你只能追着车跑了，因为SSH没得给你看。


到这里，恭喜你完成了算法组的开发环境的基本配置了，接下来就继续学习吧宝宝们。
